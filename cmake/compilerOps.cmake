...
elseif(CMAKE_C_COMPILER_ID STREQUAL "Intel" OR CMAKE_C_COMPILER_ID STREQUAL "IntelLLVM")
	...
	# Release
	set(CMAKE_C_FLAGS_RELEASE ${CMAKE_C_FLAGS_RELEASE} "-O3 -qmkl -ipo -qopt-report=3")
	set(CMAKE_CXX_FLAGS_RELEASE ${CMAKE_CXX_FLAGS_RELEASE} "-O3 -qmkl -ipo -qopt-report=3")
	set(CMAKE_Fortran_FLAGS_RELEASE ${CMAKE_Fortran_FLAGS_RELEASE} "-O3 -qmkl -ipo -qopt-report=3")
...
elseif(CMAKE_C_COMPILER_ID STREQUAL "NVHPC" OR CMAKE_C_COMPILER_ID STREQUAL "PGI")
	...
	if(USE_GPU)
		...
		if (USE_MEM_MANAGED)
			...
			set(CMAKE_C_FLAGS ${CMAKE_C_FLAGS} "-gpu=cc${GPU_CC},cuda${GPU_CUDA},managed,lineinfo -cuda -acc")
			set(CMAKE_CXX_FLAGS ${CMAKE_CXX_FLAGS} "-gpu=cc${GPU_CC},cuda${GPU_CUDA},managed,lineinfo -cuda -acc")
			set(CMAKE_Fortran_FLAGS ${CMAKE_Fortran_FLAGS} "-gpu=cc${GPU_CC},cuda${GPU_CUDA},managed,lineinfo -cuda -acc")
		else()
			message("Setting up ACC flags")
			set(CMAKE_C_FLAGS ${CMAKE_C_FLAGS} "-gpu=cc${GPU_CC},cuda${GPU_CUDA},lineinfo -cuda -acc")
			set(CMAKE_CXX_FLAGS ${CMAKE_CXX_FLAGS} "-gpu=cc${GPU_CC},cuda${GPU_CUDA},lineinfo -cuda -acc")
			set(CMAKE_Fortran_FLAGS ${CMAKE_Fortran_FLAGS} "-gpu=cc${GPU_CC},cuda${GPU_CUDA},lineinfo -cuda -acc")
		endif()
	else()
